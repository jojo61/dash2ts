...
{
  properties.emplace_back("streamurl", url);
...
bool ZatData::ReadDataJson()
{
  ...
  if (jsonString.empty())
  {
    Log(ADDON_LOG_ERROR, "Loading data.json failed.");
    return false;
  }
  ...
  {
    Log(ADDON_LOG_ERROR, "Parsing data.json failed.");
    return false;
  }
  ...
  Log(ADDON_LOG_DEBUG, "Loaded data.json.");
  return true;
}
...
bool ZatData::LoadChannels()
{
  ...
  {
    Log(ADDON_LOG_ERROR, "Failed to load channels");
    return false;
  }
...
void ZatData::myTrim( std::string &str, char* charsToRemove ) {
   for ( unsigned int i = 0; i < strlen(charsToRemove); ++i ) {
      str.erase( remove(str.begin(), str.end(), charsToRemove[i]), str.end() );
   }
}

std::string  ZatData::GetChannels()
{
  ...
  for (it = m_channelGroups.begin(); it != m_channelGroups.end(); ++it)
  {
    ...
    for (it2 = it->channels.begin(); it2 != it->channels.end(); ++it2)
    {
      ...
      if (channel.name.size()) {
        sprintf(nr,"%03d",iptvnr++);
        sprintf(id,"%d",channel.iUniqueId);
...
std::string ZatData::GetStreamUrl(Document& doc, PVRStreamProperty& properties) {
  ...
  for (Value::ConstValueIterator itr = watchUrls.Begin(); itr != watchUrls.End(); ++itr)
  {
    ...
    Log(ADDON_LOG_DEBUG, "Selected url for maxrate: %d", watchUrl["maxrate"].GetInt());
...
PVR_ERROR ZatData::GetChannelStreamProperties(const int uniqueID,
                                              PVRStreamProperty& properties)
{
  ...
  while (true) {
    ...
    Log(ADDON_LOG_INFO, "Stream properties: %s.", dataStream.str().c_str());
    int statusCode;
    ...
    Log(ADDON_LOG_INFO, "Fallback to no-drm version.");
    doc.SetNull();
...
bool ZatData::ParseRecordingsTimers(const Value& recordings, std::map<int, ZatRecordingDetails>& detailsById)
{
  ...
  while (recordingsItr != recordings.End())
  {
    ...
    {
      Log(ADDON_LOG_ERROR, "Failed to load details for recordings.");
    }
    else
    {
...
PVR_ERROR ZatData::DeleteTimer(const kodi::addon::PVRTimer& timer, bool forceDelete)
{
  ...
  if (series) {  
    ...
    if (recordingId == -1) {
      Log(ADDON_LOG_ERROR, "Did not find recording for serie %d.", timer.GetClientIndex());
      return PVR_ERROR_FAILED;
    }
  } else {
  ...
  Log(ADDON_LOG_DEBUG, "Delete timer %d", recordingId);
...
std::string ZatData::GetQualityStreamParameter(const std::string& cid, bool forceWithoutDrm, bool& requiresDrm) {
  ...
  if (iterator != m_channelsByCid.end())
  {
    ...
    if (!selectedQuality.empty()) {
      Log(ADDON_LOG_INFO, "Selected quality: %s, requiring drm: %s", selectedQuality.c_str(), requiresDrm ? "true" : "false");
...
PVR_ERROR ZatData::GetRecordingStreamProperties(const kodi::addon::PVRRecording& recording,
                                                PVRStreamProperty& properties)
{
  Log(ADDON_LOG_DEBUG, "Get url for recording %s", recording.GetRecordingId().c_str());
  PVR_ERROR ret = PVR_ERROR_FAILED;
...
{
  Log(ADDON_LOG_DEBUG, "Delete recording %s", recording.GetRecordingId().c_str());
  std::ostringstream dataStream;
...
PVR_ERROR ZatData::GetEPGTagStreamProperties(const kodi::addon::PVREPGTag& tag, PVRStreamProperty& properties)
{
  ...
  if (url.empty()) {
    ...
    if (doc.GetParseError())
    {
      Log(ADDON_LOG_ERROR, "Loading epg failed at %i", referenceTime);
      return PVR_ERROR_FAILED;
    }
    const Value& channels = doc["channels"];
    if (!channels.HasMember(channel.cid.c_str())) {
      Log(ADDON_LOG_ERROR, "Channel not found in epg.");
      return PVR_ERROR_FAILED;
    }
    const Value& channelEpg = channels[channel.cid.c_str()];
    if (!channelEpg.IsArray() || channelEpg.GetArray().Empty()) {
      Log(ADDON_LOG_ERROR, "Channel has no program at time %i.", referenceTime);
      return PVR_ERROR_FAILED;
    }
    ...
    if (url.empty()) {
      Log(ADDON_LOG_ERROR, "Could not get url for channel %s and program %i.", channel.cid.c_str(), newProgramId);
      return PVR_ERROR_FAILED;
    }
  }
...
std::string ZatData::GetStreamUrlForProgram(const std::string& cid, int programId, PVRStreamProperty& properties)
{
  Log(ADDON_LOG_DEBUG, "Get timeshift url for channel %s and program %i", cid.c_str(), programId);
...
bool ZatData::Create()
{
  Log(ADDON_LOG_DEBUG, "%s - Creating the PVR Zattoo add-on", __FUNCTION__);
