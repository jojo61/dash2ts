...
int Categories::Category(const std::string& category)
{
  ...
  if (it != m_categoriesByName.end())
    return it->second;
  Log(ADDON_LOG_INFO, "Missing category: %s", category.c_str());
...
void Categories::LoadEITCategories()
{
  ...
  if (kodi::vfs::FileExists(filePath, false))
  {
    Log(ADDON_LOG_DEBUG, "%s: Loading EIT categories from file '%s'",
        __FUNCTION__, filePath);
    kodi::vfs::CFile file;
    if (!file.OpenFile(filePath, 0))
    {
      Log(ADDON_LOG_ERROR, "%s: File '%s' failed to open", __FUNCTION__, filePath);
      return;
    }
    ...
    while (file.ReadLine(line))
    {
      ...
      if (std::regex_search(line, matches, rgx) && matches.size() == 3)
      {
        ...
        Log(ADDON_LOG_DEBUG, "%s: Add name [%s] for category %.2X",
  ...
  {
    Log(ADDON_LOG_INFO, "%s: File '%s' not found", __FUNCTION__, filePath);
  }
}
