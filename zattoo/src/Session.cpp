...
bool Session::Login(std::string u, std::string p)
{
  ...
  {
    Log(ADDON_LOG_ERROR, "Initialize session failed.");
  ...
  {
     Log(ADDON_LOG_DEBUG, "Need to login.");
     m_httpClient->ClearSession();
     
     Log(ADDON_LOG_DEBUG, "Try to login.");
     ...
     {
       Log(ADDON_LOG_DEBUG, "Login was successful.");
     }
  }
  ...
  Log(ADDON_LOG_INFO, "Current country code: %s", m_countryCode.c_str());
  Log(ADDON_LOG_INFO, "Service region country: %s",
      m_serviceRegionCountry.c_str());
  Log(ADDON_LOG_INFO, "Recall are %s",
...
bool Session::LoadAppTokenFromJson(std::string html) {
  ...
  if (basePos < 6) {
    Log(ADDON_LOG_ERROR, "Unable to find app-*.js");
    return false;
  }
  size_t endPos = html.find("\"", basePos);
  ...
  if (basePos < 6) {
    Log(ADDON_LOG_ERROR, "Unable to find token-*.json in %s", appJsPath.c_str());
    return false;
  }
...
bool Session::SendHello()
{
  ...
  Log(ADDON_LOG_DEBUG, "Send hello.");
  std::ostringstream dataStream;
  dataStream << "lang=en&";
  ...
  if (!doc.GetParseError() && doc["active"].GetBool())
  {
    Log(ADDON_LOG_DEBUG, "Hello was successful.");
    return true;
  }
  else
  {
    Log(ADDON_LOG_INFO, "Hello failed.");
    return false;
  }
}
