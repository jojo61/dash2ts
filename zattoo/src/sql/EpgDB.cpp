...
: SQLConnection("EPG-DB") {
  ...
  if (!MigrateDbIfRequired()) {
    Log(ADDON_LOG_ERROR, "%s: Failed to migrate DB to version: %i", m_name.c_str(), DB_VERSION);
  }
  Cleanup();
  ...
  if (ret != SQLITE_OK) {
    Log(ADDON_LOG_ERROR, "%s: Failed to prepare insert statement.", m_name.c_str());
  }
  ...
  if (ret != SQLITE_OK) {
    Log(ADDON_LOG_ERROR, "%s: Failed to prepare update statement.", m_name.c_str());
  }
}
...
bool EpgDB::Migrate0To1() {
  Log(ADDON_LOG_INFO, "%s: Migrate to version 1.", m_name.c_str());
...
bool EpgDB::Migrate1To2() {
  Log(ADDON_LOG_INFO, "%s: Migrate to version 2.", m_name.c_str());
...
bool EpgDB::Migrate2To3() {
  Log(ADDON_LOG_INFO, "%s: Migrate to version 3.", m_name.c_str());
...
void EpgDB::Cleanup() {
  ...
  if (!Execute("delete from EPG_INFO where END_TIME < " + std::to_string(now - 60 * 60 * 24 * 7))) {
    Log(ADDON_LOG_ERROR, "%s: Failed to clean db", m_name.c_str());
  }
}
...
EpgDBInfo EpgDB::Get(int programId) {
  ...
  if (!Query("select * from EPG_INFO where PROGRAM_ID = " + std::to_string(programId), callback)) {
    Log(ADDON_LOG_ERROR, "%s: Failed to get info from db.", m_name.c_str());
  }
...
std::list<EpgDBInfo> EpgDB::GetWithWhere(std::string where) {
  ...
  if (!Query("select * from EPG_INFO where " + where, callback)) {
    Log(ADDON_LOG_ERROR, "%s: Failed to get info from db using where part.", m_name.c_str());
  }
