...
bool SQLConnection::Open(std::string& file) {
  ...
  if(rc) {
    Log(ADDON_LOG_ERROR, "%s: Can't open database: %s", m_name.c_str(), sqlite3_errmsg(m_db));
    return false;
  }
...
bool SQLConnection::Query(std::string query, ProcessRowCallback& callback) {
  ...
  if (ret != SQLITE_OK) {
    ...
    Log(ADDON_LOG_ERROR, "%s: Query failed: %s", m_name.c_str(), sqlite3_errmsg(m_db));
    return false;
  }
  ...
  while (!done) {
    ...
    default:
      Log(ADDON_LOG_ERROR, "%s: Query failed.", m_name.c_str());
      err = true;
      done = true;
    }
  }
...
bool SQLConnection::EnsureVersionTable() {
  ...
  if (callback.GetResult() == 0) {
    Log(ADDON_LOG_INFO, "%s: SCHEMA_VERSION does not exist. Creating Table.", m_name.c_str());
...
int SQLConnection::GetVersion() {
  ...
  if (!Query("select VERSION from SCHEMA_VERSION", callback)) {
    Log(ADDON_LOG_INFO, "%s: Failed to get current version.", m_name.c_str());
    return -1;
  }
  int currentVersion = callback.GetResult();
  Log(ADDON_LOG_INFO, "%s: Current version: %d", m_name.c_str(), currentVersion);
